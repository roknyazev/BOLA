...
Ins::Ins(std::mutex *m, milliseconds p, milliseconds tcd, milliseconds pd) :
mutex(m),
packed_state(),
measure_period(p),
...
void Ins::navigation()
{
	...
	packed_state[6].raw = direct_transform(20, 0366, 370.4);
	...
	packed_state[11].raw = direct_transform(20, 0333, 0.1);
	packed_state[12].raw = tmp.raw;
	mutex->unlock();
}
